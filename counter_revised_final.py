...
#
print("lecture donnees emissions")
...
#
nbline=0
nbyearSSP=len([2005]+list(range(2010,2110,10)))
for fline in fidSSP:
  nbline=nbline+1
  if nbline > 1:
    #--reading and splitting line
...
for fline in fidAnn:
  ...
  if nbline>1:
    ...
    for ind in range(0,len(yearsann)):
      year=yearsann[ind]
      if data[1+ind]!='' and data[1+ind]!='\r\n' and data[1+ind]!='\r' and data[1+ind]!='\n':
...
for indS,source in zip(range(0,len(GDPsources)),GDPsources):
    ...
    if nbline > nblineheaderG[indS]:
      ...
      if indS!=3:
        for off,scen in zip(range(len(SSPscenarios)),SSPscenarios):
          ...
          for ind in range(0,nbyear):
            year=years[ind]
            if data[1+off*nbyear+ind]!=''and data[1+off*nbyear+ind]!='\r\n'and data[1+off*nbyear+ind]!='\r' and data[1+off*nbyear+ind]!='\n':
              value=float(data[1+off*nbyear+ind])
            else:
              value=0.0
              if verbose: print( 'Attention ',country,' GDP',' value', year, ' missing')
        ...
        for off,scen in zip(range(len(SSPscenarios)),SSPscenarios):
          ...
          for ind in range(0,4):
            year=yearsHist[ind]
            if data[2+ind]!=''and data[2+ind]!='\r\n'and data[2+ind]!='\r'and data[2+ind]!='\n':
  ...
  del GDPsource
print("calculating GDP values for PIK scenarios on country level")
#
...
print("Inverting the GDP data structure")
#
...
for source in GDPsources:
  ...
  del GDPsource
print("Converting GDP per cap from CEPII into emissions with population data")
#
...
#-----------------------------------------------------------------------------------------------------------------------
#
print("Gathering informations from the NDCs")
...
#-------------------------------------------------------------------------------------------------------------------------------------
#
print("Parameters considered in probabilistic terms")
